...
def split_text_into_chunks(text, max_tokens=8000):
    # Naive splitting based on character length (~4 characters = 1 token average)
...
GROUP_METADATA_FILE = "group_metadata.json"
DATASET_DIR = "uploaded_files"
os.makedirs(DATASET_DIR, exist_ok=True)
    ...
    for file in files:
        ...
        try:
            ...
            if file.filename.endswith(".json"):
                ...
                # ✅ Extract and save group_id and group_name from group_info.json
                if file.filename == "group_info.json":
                    try:
                        parsed_json = json.loads(file_bytes.decode("utf-8"))
                        group_id = parsed_json.get("group_id")
                        group_name = parsed_json.get("group_name")
                        if group_id and group_name:
                            with open(GROUP_METADATA_FILE, "w", encoding="utf-8") as f:
                                json.dump({
                                    "group_id": group_id,
                                    "group_name": group_name
                                }, f, indent=2)
                            logger.info(f"✅ Saved group_id and group_name to {GROUP_METADATA_FILE}")
                        else:
                            logger.warning("group_id or group_name not found in group_info.json")
                            failed_files.append({"filename": file.filename, "error": "Missing group_id or group_name"})
                            continue
                    except Exception as e:
                        logger.error(f"❌ Failed to parse group_info.json: {str(e)}")
                        failed_files.append({"filename": file.filename, "error": "Invalid group_info.json"})
                        continue
...
@app.post("/groups/userid/task")
async def query_task(
    user_phone_number: str = Form(...),
    query: str = Form("get tasks assigned for the role which we are providing and get it from all groups if user exits in all groups  ")
):
    try:
        response = user_task(query=query, user_phone_number=user_phone_number)
        return JSONResponse(content={"status": "success", "response": json.loads(response)})
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Query failed: {type(e).__name__}: {str(e)}")
