...
const AllTasks = () => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      console.log("Starting all tasks fetch...");
          ...
          if (
            ...
            const mappedTasks = parsedResponse.response.map(
              (task: any, index: number) => ({
                ...
                assigned_by: task.assigned_by || "N/A",
                priority: task.priority || "N/A",
                deadline:
                  task.deadline && task.deadline.includes(" ")
                    ? task.deadline
            ...
            console.log("Successfully fetched all tasks:", mappedTasks);
  ...
  const handleRefresh = async () => {
    console.log("Manual refresh triggered for all tasks");
  ...
  const tasksByDate = Object.entries(
    filteredTasks.reduce((acc: Record<string, Task[]>, task) => {
      ...
      const date = task.deadline.split(" ")[0] || "No date";
